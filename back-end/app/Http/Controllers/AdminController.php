...
class AdminController extends Controller
{
    ...
    /**
     * Grant Foundation Coins
     *
     * @param Request $request
     *
     * @return JSON
     */
    public function grantFoundationCoins(Request $request)
    {
        $this->validate(
            $request,
            [
        ...
        $profile = optional(User::where('uid', $uid)->first())->profile;

        if (!$profile || $profile->eth_wallet == '') {

            return response()->json(
                [
                    'message' => 'Etherium Wallet Address is required.',
                    'errors' => []
                ],
                500
            );
        ...
        try {
            ...
            return response()->json(
                [
                    'message' => 'There has been an error with transfer of ' . $amount . ' ZNX from Foundation pool.',
                    'errors' => []
                ],
                500
            );
